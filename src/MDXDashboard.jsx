...
function MiniSparkline({ data, color = COLORS.accent, height = 28, width = 80 }) {
        ...
        <div style={{ fontFamily: "'JetBrains Mono', 'SF Mono', 'Fira Code', monospace", background: `radial-gradient(ellipse at 20% 0%, rgba(15,23,60,1) 0%, ${COLORS.bg} 60%)`, color: COLORS.textPrimary, minHeight: "100vh", display: "flex", flexDirection: "column", }}>
            <style>{`
                ...
                * { box-sizing: border-box; margin: 0; padding: 0; }
                ...
                @keyframes glow { 0%, 100% { box-shadow: 0 0 4px rgba(59,130,246,0.3); } 50% { box-shadow: 0 0 12px rgba(59,130,246,0.6); } }
            ...
            <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
                ...
                <main style={{ flex: 1, overflow: "auto", padding: "20px 24px" }}>
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 20 }}>
                        ...
                        <KPICard label="Main Model" value="" icon="⬡" accent={COLORS.cyan}>
                            <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.cyan, marginTop: -4, wordBreak: "break-all", lineHeight: 1.3 }}>
                                {MOCK_SUMMARY.mainModel}
                            </div>
                        </KPICard>
                    </div>
                    {/* Override KPICard for Main Model since it needs custom content */}
                    {/* Re-render the 5th card properly */}
                            ...
                            {MOCK_AGENTS.map(a => (
                                <GlassCard key={a.id} hover onClick={() => setSelectedAgent(a)} style={{ minWidth: 180, flex: "0 0 auto" }} padding="10px 14px" >
                                    ...
                                    <div style={{ display: "flex", gap: 12, fontSize: 10, color: COLORS.textMuted }}>
                                        ...
                                        <span style={{ color: a.errors24h > 0 ? COLORS.error : COLORS.textMuted }}>
                                            {a.errors24h} errors
                                        </span>
                            ...
                            {MOCK_USAGE_BREAKDOWN.map(u => (
                                <GlassCard key={u.model} hover padding="12px 14px">
                                    ...
                                    <div style={{ width: "100%", height: 3, borderRadius: 2, background: "rgba(255,255,255,0.04)", marginBottom: 8, overflow: "hidden" }}>
                                        <div style={{ width: `${u.pct}%`, height: "100%", borderRadius: 2, background: `linear-gradient(90deg, ${COLORS.accent}, ${COLORS.cyan})` }} />
                                    </div>
                                    <div style={{ display: "flex", justifyContent: "space-between", fontSize: 10, color: COLORS.textMuted }}>
            ...
            <Drawer open={!!selectedSession} onClose={() => setSelectedSession(null)} title={`Session: ${selectedSession?.id || ""}`}>
                {selectedSession && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                            <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 8, padding: 12, border: `1px solid ${COLORS.border}` }}>
                            ...
                            <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 8, padding: 12, border: `1px solid ${COLORS.border}` }}>
                                ...
                                <div style={{ fontSize: 12, color: COLORS.textPrimary, fontWeight: 500 }}>{selectedSession.agent}</div>
                            </div>
                            <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 8, padding: 12, border: `1px solid ${COLORS.border}` }}>
                                <div style={{ fontSize: 10, color: COLORS.textMuted, marginBottom: 4 }}>MODEL</div>
                                <div style={{ fontSize: 11, color: COLORS.cyan }}>{selectedSession.model}</div>
                            </div>
                            <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 8, padding: 12, border: `1px solid ${COLORS.border}` }}>
                                <div style={{ fontSize: 10, color: COLORS.textMuted, marginBottom: 4 }}>TOKENS 24H</div>
                                <div style={{ fontSize: 14, color: COLORS.textPrimary, fontWeight: 600 }}>{fmt(selectedSession.tokens24h)}</div>
                            </div>
                        </div>

                        <div style={{ background: "rgba(255,255,255,0.02)", borderRadius: 8, padding: 12, border: `1px solid ${COLORS.border}` }}>
                            <div style={{ fontSize: 10, color: COLORS.textMuted, marginBottom: 4 }}>TIMELINE</div>
                            <div style={{ fontSize: 11, color: COLORS.textSecondary }}>Started: {timeAgo(selectedSession.started)}</div>
                            <div style={{ fontSize: 11, color: COLORS.textSecondary }}>Last seen: {timeAgo(selectedSession.lastSeen)}</div>
                        </div>
                        <div>
                            <div style={{ fontSize: 11, fontWeight: 600, color: COLORS.textPrimary, marginBottom: 8 }}>Recent Runs</div>
                            {MOCK_RUNS.slice(0, 4).map(r => (
                                <div key={r.id} style={{ padding: "8px 0", borderBottom: `1px solid rgba(255,255,255,0.03)`, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    <div>
            ...
            {/* ── Run Detail Drawer ──────────────────────── */}
            <Drawer open={!!selectedRun} onClose={() => setSelectedRun(null)} title={`Run: ${selectedRun?.id || ""}`}>
                {selectedRun && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                            {[
                                { label: "Status", content: <StatusPill status={selectedRun.status} /> },
                                { label: "Source", content: <SourceBadge source={selectedRun.source} /> },
                                { label: "Model", content: <span style={{ fontSize: 11, color: COLORS.cyan }}>{selectedRun.model}</span> },
                                { label: "Duration", content: <span style={{ fontSize: 13, color: COLORS.textPrimary, fontWeight: 600 }}>{durFmt(selectedRun.duration)}</span> },
                                { label: "Context", content: selectedRun.contextPct > 0 ? <ContextBar pct={selectedRun.contextPct} /> : <span style={{ fontSize: 11, color: COLORS.textMuted }}>—</span> },
                                { label: "Tokens", content: <span style={{ fontSize: 11, color: COLORS.textSecondary }}>{fmt(selectedRun.tokensIn)} → {fmt(selectedRun.tokensOut)}</span> },
                            ].map((item, i) => (
                                <div key={i} style={{ background: "rgba(255,255,255,0.02)", borderRadius: 8, padding: "8px 10px", border: `1px solid ${COLORS.border}` }}>
                                    <div style={{ fontSize: 9, color: COLORS.textMuted, marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }}>{item.label}</div>
                                    {item.content}
                                </div>
                            ))}
                        </div>

                        {selectedRun.finishReason && (
                            <div style={{ fontSize: 11, color: COLORS.textMuted }}>
                                Finish reason: <span style={{ color: selectedRun.finishReason === "error" ? COLORS.error : COLORS.textSecondary }}>{selectedRun.finishReason}</span>
                            </div>
                        )}

                        {/* Accordion sections */}
                        {["Transcript", "Tool Calls", "Errors", "Artifacts"].map(section => (
                            <details key={section} style={{ borderRadius: 8, border: `1px solid ${COLORS.border}`, overflow: "hidden" }}>
                                <summary style={{ padding: "10px 12px", cursor: "pointer", fontSize: 12, fontWeight: 500, color: COLORS.textPrimary, background: "rgba(255,255,255,0.02)", listStyle: "none", display: "flex", alignItems: "center", gap: 6, }}><span style={{ fontSize: 10, color: COLORS.textMuted }}>▶</span> {section}</summary>
                                <div style={{ padding: 12, fontSize: 11, color: COLORS.textMuted, background: "rgba(0,0,0,0.1)" }}>
                                    {section === "Transcript" && <div style={{ fontFamily: "inherit", lineHeight: 1.6, whiteSpace: "pre-wrap" }}>
                                        [System] Initializing {selectedRun.model}...{" "}
                                        [User] {selectedRun.label}{" "}
                                        [Assistant] Processing request with context window at {selectedRun.contextPct}%...{" "}
                                        [Assistant] Generating response...
                                    </div>}
                                    {section === "Tool Calls" && <div>
                                        <div style={{ padding: "6px 0", borderBottom: `1px solid rgba(255,255,255,0.03)` }}>
                                            <span style={{ color: COLORS.accentBright }}>file_read</span>
                                            <span style={{ color: COLORS.textMuted }}>→ {"{ path: '/src/index.ts' }"}</span>
                                        </div>
                                        <div style={{ padding: "6px 0" }}>
                                            <span style={{ color: COLORS.accentBright }}>web_search</span>
                                            <span style={{ color: COLORS.textMuted }}>→ {"{ query: 'rate limiting patterns' }"}</span>
                                        </div>
                                    </div>}
                                    {section === "Errors" && (selectedRun.status === "failed" ? <div style={{ color: COLORS.error }}>Error: Context window exceeded maximum token limit</div> : <div>No errors</div>
                                    )}
                                    {section === "Artifacts" && <div>No artifacts generated</div>}
                                </div>
                            </details>
                        ))}
                    </div>
            ...
            {/* ── Agent Detail Drawer ────────────────────── */}
            <Drawer open={!!selectedAgent} onClose={() => setSelectedAgent(null)} title={`Agent: ${selectedAgent?.name || ""}`}>
                {selectedAgent && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                            {[
                                { label: "Type", content: <SourceBadge source={selectedAgent.type} /> },
                                { label: "Status", content: <StatusPill status={selectedAgent.status} /> },
                                { label: "Model", content: <span style={{ fontSize: 11, color: COLORS.cyan }}>{selectedAgent.model}</span> },
                                { label: "Last Run", content: <span style={{ fontSize: 11, color: COLORS.textSecondary }}>{timeAgo(selectedAgent.lastRun)}</span> },
                                { label: "Runs 24h", content: <span style={{ fontSize: 14, fontWeight: 600, color: COLORS.textPrimary }}>{selectedAgent.runs24h}</span> },
                                { label: "Errors 24h", content: <span style={{ fontSize: 14, fontWeight: 600, color: selectedAgent.errors24h > 0 ? COLORS.error : COLORS.success }}>{selectedAgent.errors24h}</span> },
                            ].map((item, i) => (
                                <div key={i} style={{ background: "rgba(255,255,255,0.02)", borderRadius: 8, padding: "8px 10px", border: `1px solid ${COLORS.border}` }}>
                                    <div style={{ fontSize: 9, color: COLORS.textMuted, marginBottom: 4, textTransform: "uppercase", letterSpacing: 0.5 }}>{item.label}</div>
                                    {item.content}
